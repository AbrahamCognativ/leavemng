...
export class ProfileComponent implements OnInit {
  ...
  formatImageUrl(url: string): string {
    ...
    const baseUrlWithoutApi = this.baseUrl.split('/api')[0];
    
    // If it's a path starting with /uploads, use the base URL without API path
    if (url.startsWith('/uploads/')) {
      return `${baseUrlWithoutApi}${url}`;
    }
    
    // If it's another relative path with leading slash, use the full API base URL
    if (url.startsWith('/')) {
