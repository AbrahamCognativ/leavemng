...
def test_send_invite_email():
    ...
    with patch('app.utils.email.send_email') as mock_send:
        email.send_invite_email('test@example.com', 'http://invite-link', mock_request)
...
def test_send_leave_request_notification():
    ...
    mock_request = MagicMock()
    with patch('app.utils.email.send_email') as mock_send:
        email.send_leave_request_notification('manager@example.com', 'Alice', 'Annual Leave: 2025-05-05', mock_request)
        mock_send.assert_called_once()
        args, kwargs = mock_send.call_args
...
def test_send_leave_approval_notification():
    ...
    mock_request = MagicMock()
    with patch('app.utils.email.send_email') as mock_send:
        email.send_leave_approval_notification('user@example.com', 'Annual Leave: 2025-05-05', True, mock_request)
        mock_send.assert_called_once()
        args, kwargs = mock_send.call_args
