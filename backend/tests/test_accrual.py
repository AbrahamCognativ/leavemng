...
class DummyPolicy:
    def __init__(self, org_unit_id, accrual_frequency, accrual_amount_per_period, leave_type=None, leave_type_id=None):
...
def test_accrue_leave_balances_monthly(dummy_db):
    ...
    policy = DummyPolicy(org_unit_id=1, accrual_frequency=accrual.AccrualFrequencyEnum.monthly, accrual_amount_per_period=2, leave_type=DummyLeaveType(code='annual'), leave_type_id=1)
...
def test_accrue_leave_balances_gender_eligibility(dummy_db):
    ...
    policy = DummyPolicy(org_unit_id=1, accrual_frequency=accrual.AccrualFrequencyEnum.monthly, accrual_amount_per_period=2, leave_type=DummyLeaveType(code='maternity'), leave_type_id=1)
    user = DummyUser(id=1, org_unit_id=1, extra_metadata='{"gender": "male"}')
    dummy_db.query.side_effect = lambda model: MagicMock(all=lambda: [policy] if model.__name__ == 'LeavePolicy' else [user] if model.__name__ == 'User' else [], filter_by=lambda **kwargs: MagicMock(first=lambda: None))
    accrual.accrue_leave_balances(dummy_db)
