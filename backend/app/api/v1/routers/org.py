...
def get_org_chart(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """
    ...
    try:
        ...
        root_units = db.query(OrgUnit).filter(OrgUnit.parent_unit_id == None).all()
        ...
        if result:
            print(f"First unit: {result[0]['name'] if 'name' in result[0] else 'Unknown'}")
        ...
        raise HTTPException(status_code=500, detail="Failed to generate organization chart")
    ...
    try:
        ...
        if hasattr(unit, 'users'):
            ...
            try:
                ...
                print(f"Found {len(unit_users)} users for org unit {unit.name}")
            except Exception as e:
                print(f"Error loading users for org unit {unit.name}: {str(e)}")
                unit_users = []

            for user in unit_users:
                try:
                    role_key = user.role_title if hasattr(user, 'role_title') else 'Unknown Role'
                    ...
                    user_name = user.name if hasattr(user, 'name') else f"User {user.id}"
                    users_by_role[role_key]["users"].append({
                        "id": str(user.id),
                        "name": user_name
                    })
                except Exception as user_err:
                    print(f"Error processing user {getattr(user, 'id', 'unknown')}: {str(user_err)}")
                ...
                try:
                    if hasattr(user, 'manager_id') and user.manager_id:
                        ...
                        if manager and manager.org_unit_id == unit.id:
                            # If manager is in the same org unit, link their roles
                            manager_role_key = manager.role_title if hasattr(manager, 'role_title') else 'Unknown Role'
                            user_role_key = user.role_title if hasattr(user, 'role_title') else 'Unknown Role'

                            if manager_role_key in users_by_role and user_role_key in users_by_role:
                                # Add this role as a child of the manager's role if not already added
                                if not any(child["id"] == users_by_role[user_role_key]["id"] for child in users_by_role[manager_role_key]["children"]):
                                    users_by_role[manager_role_key]["children"].append(users_by_role[user_role_key])
                except Exception as mgr_err:
                    print(f"Error processing manager relationship for user {getattr(user, 'id', 'unknown')}: {str(mgr_err)}")
        else:
            print(f"Warning: 'users' relationship not loaded for org unit {unit.name}")

        # Add roles to the unit
        for role_dict in users_by_role.values():
            # Only add roles that aren't already child of another role
            if not any(role_dict["id"] == child["id"] for role in users_by_role.values() for child in role["children"]):
                unit_dict["children"].append(role_dict)

        # Add child units
        try:
            children = db.query(OrgUnit).filter(OrgUnit.parent_unit_id == unit.id).all()
            for child in children:
                child_dict = build_org_unit_dict(child, db)
                unit_dict["children"].append(child_dict)
        except Exception as child_err:
            print(f"Error loading child units for {unit.name}: {str(child_err)}")

        return unit_dict
    except Exception as e:
        import traceback
        print(f"Error in build_org_unit_dict for unit {getattr(unit, 'name', 'unknown')}: {str(e)}")
        print(traceback.format_exc())
