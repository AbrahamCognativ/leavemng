...
@router.put("/{policy_id}", response_model=LeavePolicyRead, tags=["leave-policy"], dependencies=[Depends(require_role(["HR", "Admin"]))])
def update_leave_policy(policy_id: uuid.UUID, policy: LeavePolicyCreate, db: Session = Depends(get_db)):
