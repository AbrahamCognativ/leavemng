...
def auto_reject_old_pending_leaves():
    ...
    try:
        ...
        if not old_pending:
            log_audit(db, "Auto-Reject Pending Leaves", "No old pending leaves found")
        else:
            for req in old_pending:
                ...
                if user:
                    leave_details = {
                        "Type": next((f"{lt.code.value.capitalize()} leave" for lt in db.query(LeaveType).all() if lt.id == req.leave_type_id and lt.code.value in ['sick', 'annual', 'compassionate', 'maternity', 'paternity']), 'Unknown'),
                    ...
                    try:
                        ...
                        log_audit(db, "Auto-Reject Pending Leaves", f"Error sending auto-rejection email: {e}")
            log_audit(db, "Auto-Reject Pending Leaves", f"Auto-rejected {len(old_pending)} old pending leaves")
    except Exception as e:
